...
| Area | What landed |
|------|-------------|
| **Logging** | Library takes `ScanParams.Logger *slog.Logger` (nil → `slog.Default()`). Scan summary is `Info`; per-device finds, claimed skips, and probe errors are `Debug`. No `log.Printf` in `internal/snmpdiscovery`. |
...
| **Stale CLI copy** | `tools/snmp-discovery/` is an old duplicate. PR surface is `cmd/snmp-discovery` + `internal/snmpdiscovery`. |

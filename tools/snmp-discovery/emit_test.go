...
func TestFileSDUsesParamLabelsNotCommunity(t *testing.T) {
	...
	l := groups[0].Labels
	if l["__param_module"] != "if_mib,nokia_srlinux" || l["__param_auth"] != "public_v2" {
		t.Fatalf("labels=%v", l)
	}
	if _, ok := l["module"]; ok {
...
func TestAlloyYAMLHasNamedAuthNoCommunity(t *testing.T) {
	...
	for _, want := range []string{"name: leaf1", "address: 172.20.20.3", "module: if_mib,nokia_srlinux", "auth: public_v2"} {

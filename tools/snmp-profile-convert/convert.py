...
writes ``snmp/modules/`` which **is** the Alloy library after that. Do not
...
Layout:
  ...
  snmp/snmp-network.yml                # concat for Alloy's single config_file
...
# Not a VITAL_TAG — profiles keep tag: CPU; we remap so Alloy does not mix them.
...
def snmpv2_device_info_metric(extra_lookups: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "help": (
            ...
            "required: snmp_exporter skips lookups with empty labels. Alloy copies "
            "snmp_device_info → sysName and drops snmp_scalar_index."
        ),
...
def classify_module(name: str) -> str:
    ...
    if (
        ...
        or n in HOT_VENDOR_MODULES
        or n.endswith("_hot")
        or n.endswith("_identity")
    ):
        return "hot"
    if n in TOPOLOGY_MODULES or TOPOLOGY_NAME_RE.search(n):
        return "topology"
    return "cold"
...
def build_module_tiers_doc(modules: dict[str, Any]) -> dict[str, Any]:
    ...
    for n in sorted(modules):
        tiers[classify_module(n)].append(n)
    return {
        "description": (
            "Scrape tiers for Alloy staggered polling. "
            "hot≈60s alerting/troubleshooting; cold≈metadata + walks that miss 60s; "
            "topology=optional LLDP/CDP experiments."
        ),
        "tiers": tiers,
    }
...
# (snmp_exporter dynamic filters). Alloy relabel cannot drop on gauge *values*.
...
def apply_if_admin_up_filter(
    ...
    # Runtime snmp.yml shape (Alloy / snmp_exporter): filters is a list.
...
def concat_snmp_network(
    ...
    header = (
        ...
        "# Alloy prometheus.exporter.snmp still takes a single config_file; this file is\n"
        "# the concatenation. Prefer editing split modules and re-running convert.py.\n"
        "# Use config_merge_strategy = \"merge\" so embedded if_mib remains available.\n"
    )
...
def main() -> int:
    ...
    fp = build_fingerprinters(index)  # also fills extends_modules + module_chain on index

...
func TestLogsFromRecord(t *testing.T) {
	...
	require.Equal(t, "spine1", mustAttr(t, lr.Attributes(), "device_name"))
	...
	require.Equal(t, "spine1", parsed["device_name"])
...
func TestListenChanged(t *testing.T) {
	base := ListenConfig{ListenAddress: "0.0.0.0:1620", Communities: []string{"public"}}
	same := base
	same.IncludeCommunity = true
	require.False(t, ListenChanged(base, same))

	moved := base
	moved.ListenAddress = "0.0.0.0:11620"
	require.True(t, ListenChanged(base, moved))
}
